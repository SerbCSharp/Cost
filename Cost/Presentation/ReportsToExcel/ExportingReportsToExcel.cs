...
namespace Cost.Presentation.ReportsToExcel
{
    public class ExportingReportsToExcel
    {
        ...
        public void Expense(IEnumerable<Expense> cost) // Стоимость строительства
        {
            ...
            sheet.Cells[1, 18].Value = "ContractId";
            ...
            foreach (var item in cost)
            {
                ...
                sheet.Cells[row, column + 13].Value = item.SecurityDeposit;
                ...
                sheet.Cells[row, column + 18].Value = item.ContractId;
        ...
        public void ReconciliationStatement(IEnumerable<ReconciliationStatement> reconciliationStatement)
        {
            string filePath = "C:\\Cost\\Transcript.xlsx";
            using var package = new ExcelPackage();
            
            var sheet = package.Workbook.Worksheets.Add("Акт сверки по договору");
        ...
        public void CashFlowSource(IEnumerable<CashFlow> cashFlow, string organization) // ДДС Source
        {
            ...
            foreach (var item in cashFlow)
            {
                ...
                sheetSource.Cells[row, column + 4].Value = item.Debit;
                sheetSource.Cells[row, column + 5].Value = item.Credit;
        ...
        public void CashFlow(IEnumerable<CashFlow> cashFlow, decimal startBalance, string organization, DateOnly startDate, DateOnly endDate) // ДДС
        {
            ...
            sheetFinal.Cells[4, 1].Value = "Показатель";
            ...
            foreach (var item in cashFlow)
            {
                ...
                {
                    row++;
                    sheetFinal.Cells[row, column].Value = item.TypeOfActivity;
                    sheetFinal.Cells[row, column, row, column + 1].Style.Font.Bold = true;
                    sheetFinal.Cells[row, column, row, column + 1].Style.Font.Size = 12;
                    sheetFinal.Cells[row, column + 1].Value = item.SumTypeOfActivity;
                ...
                sheetFinal.Cells[row + 1, column + 1].Value = item.Debit - item.Credit - item.IndirectCosts;
                endBalance = endBalance + item.Debit - item.Credit - item.IndirectCosts;
                ...
                if (item.TypeOfActivity != "Финансовая деятельность")
                {
                    ...
                    sheetFinal.Cells[row + 2, column + 1].Value = item.Debit;
                    sheetFinal.Cells[row + 3, column].Value = "Оплаты (прямые расходы)".PadLeft("Оплаты (прямые расходы)".Length + 10);
                    sheetFinal.Cells[row + 3, column + 1].Value = item.Credit;
            ...
            sheetFinal.Cells[1, 1, row + 1, 2].AutoFitColumns();
            sheetFinal.Cells[5, 2, row + 1, 2].Style.Numberformat.Format = "### ### ### ##0.00";

            range = sheetFinal.Cells[4, 1, row + 1, 2];
            range.Style.Border.Top.Style = ExcelBorderStyle.Thin;
            range.Style.Border.Bottom.Style = ExcelBorderStyle.Thin;
            range.Style.Border.Left.Style = ExcelBorderStyle.Thin;
            range.Style.Border.Right.Style = ExcelBorderStyle.Thin;
            ...
            sheetFinal.Cells[row + 1, 1, row + 1, 2].Style.Font.Bold = true;
        ...
        {
            string filePath = $"C:\\Cost\\{organization}HowMuchIsLeftToPayExtra.xlsx";
            using var package = new ExcelPackage();

            var sheet = package.Workbook.Worksheets.Add("Оплата по счетам");
            sheet.Cells.Style.Font.Name = "Calibri";
            sheet.Cells.Style.Font.Size = 11;
            sheet.View.FreezePanes(2, 1);
            ...
            sheet.Cells[1, 3].Value = "Договор";
            sheet.Cells[1, 4].Value = "Контрагент";
            sheet.Cells[1, 5].Value = "Сумма счета";
            sheet.Cells[1, 6].Value = "Оплатили";
            sheet.Cells[1, 7].Value = "Привязка оплаты к счету";
            sheet.Cells[1, 1, 1, 7].Style.Font.Bold = true;
            sheet.Cells[1, 1, 1, 7].Style.HorizontalAlignment = ExcelHorizontalAlignment.Center;

            var row = 2;
            var column = 0;
            foreach (var item in howMuchIsLeftToPayExtra)
            {
                sheet.Cells[row, column + 1].Value = item.Number;
                sheet.Cells[row, column + 2].Value = item.Date;
                sheet.Cells[row, column + 3].Value = item.Contract;
                sheet.Cells[row, column + 4].Value = item.Contractor;
                sheet.Cells[row, column + 5].Value = item.SupplierPaymentInvoiceAmount;
                sheet.Cells[row, column + 6].Value = item.PaymentAmount;
                sheet.Cells[row, column + 7].Value = item.PaymentId;
                row++;
            }

            sheet.Cells[row, 2, row, 7].Style.Font.Bold = true;
            sheet.Cells[1, 1, row, 17].AutoFitColumns();
            sheet.Cells[2, 2, row, 2].Style.Numberformat.Format = "dd.mm.yyyy";
            sheet.Cells[2, 5, row, 6].Style.Numberformat.Format = "### ### ### ##0.00";
        ...
        public void ExpensesUnderIncomeContracts(IEnumerable<ExpensesUnderIncomeContracts> income) // Затраты по доходным договорам
        {
            string filePath = "C:\\Cost\\ExpensesUnderIncomeContracts.xlsx";
            using var package = new ExcelPackage();

            var sheet = package.Workbook.Worksheets.Add("Затраты по доходным договорам");
            sheet.Cells.Style.Font.Name = "Calibri";
            sheet.Cells.Style.Font.Size = 11;
            sheet.View.FreezePanes(2, 1);
